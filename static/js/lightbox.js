...
;
//  Drupal.behaviors.st7 = {
/* Feature Stories - rotating files/images 
 * Class: .view-feature-story.view-display-id-block 
 * This JS rotates the parent class div - fades it in among others.
 *
 */
jQuery(document).ready(function($) {
  // Get the number of featured items
  var num_stories = $('.view-feature-story.view-display-id-block .view-content .fs-row').size();
  if (num_stories == 1) { $('.side-controls').css('display', 'none'); }
  
  var active_story = 0;
  active_story = $('.view-feature-story.view-display-id-block .fs-row.active').index();
  var running = true;

  
  if (num_stories > 1) {
    // view view-feature-story view-id-feature_story view-display-id-block
    $('.view-feature-story.view-display-id-block')
      .everyTime('7s', 'feature-rotation', function() {setFeature((active_story+1) % num_stories)}, 0)
      .hover(
        function() { if(running)
          $(this).stopTime('feature-rotation');
        },
        function() { if(running)
          $(this).everyTime('4s', 'feature-rotation', function() {setFeature((active_story+1) % num_stories)}, 0);
        }
      );
  }
  ...
  $('.next').click(function() {
    setFeature((active_story+1) % num_stories);
  });
  
  $('.previous').click(function() {
    if (active_story == 0) {
      setFeature(num_stories-1);
    } else {
      setFeature(active_story-1);
    }
  });
  

  // Sets the active class, and fades in and out the inactive areas.
  function setFeature(i) {
    if(i != active_story) {
        $('.view-feature-story.view-display-id-block .fs-row').addClass('inactive').removeClass('active');
        $('.view-feature-story.view-display-id-block .fs-row:eq('+i+')').removeClass('inactive').addClass('active');
        active_story = i;
    }
  };
